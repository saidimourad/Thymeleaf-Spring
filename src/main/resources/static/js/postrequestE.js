...
////
		var customerRow = '<tr>' +
							...
							'<td>' + formCustomer.articleE.id_art + '</td>' +
							'<td>' + formCustomer.D_art + '</td>' +
							'<td>' + formCustomer.qteEntre+  '</td>' +
							'<td>' + formCustomer.prixEntre+ '</td>' +
							'<td class="totalprice">' + formCustomer.qteEntre * formCustomer.prixEntre+ '</td>' +
	...
	$('#postCustomersBtnn').click(function(){
		...
		var formd = {
				 date_entree : $('#date').val(),
				 prixTotal_entree:$('#totG').val(),
		    	  ref_entree : $('#ref').val(),
		    	  fournisseur:
		    	  {
		    	  id_fr: $('#fournisseur').val()
		    	  },
		        magasin:{
    ...
    function ajaxPost(){
    	...
    	$.ajax({
			...
			accept: 'text/plain',
			url : window.location + "/api/entree/save",
...
//	data : JSON.stringify(Edata),
    ...
    function ajaxPostDetail(){
    	...
    	$.ajax({
			...
			accept: 'text/plain',
			url : window.location + "/api/entree/save2",
